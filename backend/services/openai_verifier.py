"""
OpenAI GPT-4 Vision-Powered Answer Verification
Verifies uncertain OMR detections with GPT-4 Vision
"""
...
class OpenAIVerifier:
    """
    OpenAI GPT-4 Vision yordamida shubhali javoblarni tekshirish
    """
    ...
    def __init__(
        ...
        logger.info(f"OpenAI Verifier initialized with model: {model}")
    ...
    def verify_uncertain_answers(
        ...
        """
        logger.info("Starting OpenAI GPT-4 Vision verification...")
        ...
        logger.info(f"Verifying {len(questions_to_verify)} answers with OpenAI GPT-4 Vision...")
        ...
        for question in questions_to_verify:
            try:
                ...
                logger.error(f"OpenAI verification failed for Q{question['questionNumber']}: {e}")
        
        logger.info(
            f"OpenAI verification complete: {verified_count} verified, "
    ...
    def _extract_question_region(
        ...
        # Resize if too small (GPT-4 Vision works better with larger images)
        if crop.shape[0] < 200 or crop.shape[1] < 200:
            scale = max(200 / crop.shape[0], 200 / crop.shape[1])
...
**Rules:**
1. Look for the DARKEST bubble with the most ink/pencil marks
2. Ignore light marks, scratches, smudges, or stray marks outside bubbles
3. If multiple bubbles are marked, choose the DARKEST and most completely filled one
4. If no bubble is clearly marked, respond with "NONE"
5. Be decisive - choose the most likely answer based on visual evidence
