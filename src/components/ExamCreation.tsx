...
const ExamCreation: React.FC<ExamCreationProps> = ({
	...
	const validateStep = (step: number): boolean => {
		switch (step) {
			...
			case 2:
				...
				const hasEmptySubjects = examData.subjects.some(
					subject => !subject.name.trim() || subject.sections.length === 0
				)

				if (hasEmptySubjects) {
					setToast({
						message: "Barcha mavzular va bo'limlarni to'ldiring!",
						type: 'error',
					})
					return false
				}
				...
				const hasEmptySections = examData.subjects.some(subject =>
					subject.sections.some(
						section => !section.name.trim() || section.questionCount < 1
					)
				)

				if (hasEmptySections) {
					setToast({
						message: "Barcha bo'limlarni to'g'ri to'ldiring!",
						type: 'error',
					})
					return false
				}
	...
	const saveExam = async () => {
		...
		try {
			...
			console.error('Imtihonni saqlashda xatolik:', error)
						...
						<div className='space-y-6'>
							<div>
								...
								<input
									...
									value={examData.name}
									onChange={e =>
										setExamData(prev => ({ ...prev, name: e.target.value }))
									}
							...
							<div>
								...
								<input
									...
									value={examData.date}
									onChange={e =>
										setExamData(prev => ({ ...prev, date: e.target.value }))
									}
								/>
							</div>
												...
												>
													<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-3'>
														...
														<div className='flex items-end'>
															<button
																onClick={() =>
																	deleteSection(subject.id, section.id)
																}
							...
							<div>
								...
								<div className='space-y-4'>
									{examData.subjects.map((subject, index) => {
										...
										const maxScore = subject.sections.reduce(
											(sum, section) =>
												sum + section.questionCount * section.correctScore,
											0
										)
															...
															<span>
																...
																<span className='ml-2 text-gray-500'>
																	...
																	{section.questionType === 'multiple-choice'
																		? 'Bir tanlov'
																		: section.questionType === 'multiple-select'
																		? "Ko'p tanlov"
																		: 'Ochiq javob'}
																	)
																</span>
